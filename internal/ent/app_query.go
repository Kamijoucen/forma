...
// QuerySchemaDefs chains the current query on the "schemaDefs" edge.
func (_q *AppQuery) QuerySchemaDefs() *SchemaDefQuery {
...
// WithSchemaDefs tells the query-builder to eager-load the nodes that are connected to
// the "schemaDefs" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AppQuery) WithSchemaDefs(opts ...func(*SchemaDefQuery)) *AppQuery {
	query := (&SchemaDefClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSchemaDefs = query
	return _q
}
...
func (_q *AppQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*App, error) {
	...
	if query := _q.withSchemaDefs; query != nil {
		if err := _q.loadSchemaDefs(ctx, query, nodes,
			func(n *App) { n.Edges.SchemaDefs = []*SchemaDef{} },
			func(n *App, e *SchemaDef) { n.Edges.SchemaDefs = append(n.Edges.SchemaDefs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AppQuery) loadSchemaDefs(ctx context.Context, query *SchemaDefQuery, nodes []*App, init func(*App), assign func(*App, *SchemaDef)) error {
	...
	query.withFKs = true
	query.Where(predicate.SchemaDef(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(app.SchemaDefsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.app_schema_defs
		if fk == nil {
			return fmt.Errorf(`foreign-key "app_schema_defs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "app_schema_defs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
